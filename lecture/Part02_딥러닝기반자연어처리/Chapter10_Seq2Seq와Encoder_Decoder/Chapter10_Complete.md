...
[05_beam_search.py](05_beam_search.py)는 beam 크기 2로 다음 과정을
직접 실행합니다.

1. 현재 beam의 각 문장에 가능한 다음 토큰을 붙입니다.
2. 토큰의 로그 확률을 기존 누적 점수에 더합니다.
3. 전체 후보 중 점수가 높은 두 개만 남깁니다.
4. `<EOS>`가 나오거나 최대 길이에 도달할 때까지 반복합니다.

```bash
python 05_beam_search.py
```

각 단계에서 살아남은 후보가 출력되므로 Greedy Search와 달리 여러 경로를
동시에 유지한다는 점을 확인할 수 있습니다. 마지막에는 길이 정규화 점수도
함께 출력합니다.
